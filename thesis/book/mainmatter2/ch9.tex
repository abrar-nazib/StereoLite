...
% Ported from old mainmatter/ch9.tex with chapter reference remapping:
% old Chapter 8 (results) -> new Chapter 6 via \ref{ch:results}
% All numbers and labels preserved; label ch:conclusion added.
% =====================================================================
...
reasoning. The rectification tolerance measured in \ref{ch:results} holds only
...
pairs, using the teacher pipeline built for this project. The
cross-domain evidence in \ref{ch:results} points to the training
pipeline as the largest remaining lever: failure concentrates in a scene type real-data
...
(from \ref{ch:results}), so the next step is a full accuracy audit of the quantized
...
reconstruction path discussed in \ref{ch:results} invites a mapping extension that fuses
...
% SOURCES: old mainmatter/ch9.tex (all three sections: Conclusion, Limitations, Future Works)
% DROPPED: none (entire content ported)
% MOVED-AWAY: none (this chapter owns all three sections in the new structure)
