"""StereoLite_yolo_geomctx: context-gated slanted tile recurrent stereo.

This sibling keeps the YOLO26 matching encoder and RAFT-style left-image
context stream from `StereoLite_yolo_ctx`, then tests the geometry-aware
extensions proposed for the thesis model:

  1. Slant-aware local lookup: sx/sy affect the sampled support patch
     during refinement, not only the cross-scale upsample.
  2. Context-gated rescue: a tiny local stereo correction from the lookup
     is blended with the ConvGRU proposal using context, confidence, and
     disagreement cues.
...
Pipeline:
  ...
  Cost-volume init at 1/16 -> h overwritten with ctx16 -> slant-aware,
  context-gated TileRefineCtx(fL16, fR16, ctx=ctx16) × N16.
...
Budget target: still ~2-3 M trainable, with extra runtime from the 3x3
slanted lookup. Use `slant_patch_radius=0` for a cheap center-only ablation.
"""
...
class ContextEncoder(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # x is in [0, 255]; normalise to [0, 1] like the matching encoder.
        x = x / 255.0
        x = self.stem(x)         # 1/2
        x = self.s4(x)           # 1/4
...
@dataclass
class StereoLiteYoloGeomCtxConfig:
    base_ch: int = 24
    tile_feat_ch: int = 32
    ctx_ch: int = 32
    iters_16: int = 2
    ...
    slant_patch_radius: int = 1
    backbone: str = "yolo26s"     # mid-tier default; can switch to yolo26n
    backbone_pretrained: bool = True


class StereoLiteYoloGeomCtx(nn.Module):
    def __init__(self, cfg: StereoLiteYoloGeomCtxConfig | None = None):
        super().__init__()
        self.cfg = cfg or StereoLiteYoloGeomCtxConfig()
        ...
        self.refine_16 = TileRefineCtx(
            ...
            patch_radius=self.cfg.slant_patch_radius)
...
# Compatibility aliases for quick swaps in existing ablation scripts.
