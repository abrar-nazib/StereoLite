"""StereoLite_yolo_ctx_sharp: gated context stereo with sharp refinement.
...
Pipeline:
  ...
  Final: ConvexUpsample 1/4 -> 1/2 -> full, followed by a lightweight
  boundary-aware residual tail.
...
class SharpRefineTail(nn.Module):
    """Small full-resolution residual tail for sharper disparity edges.

    The stereo core still owns the disparity estimate. This tail only predicts
    a bounded residual and an edge mask from the left image, full disparity,
    and upsampled 1/2 features. The mask makes the residual concentrate near
    boundaries instead of changing smooth interior regions everywhere.
    """
    ...
    def __init__(self, feat_ch: int, hidden: int = 32,
        ...
        self.res_head = nn.Conv2d(hidden, 1, 1)

    def forward(self, disp: torch.Tensor, left: torch.Tensor,
                feat2: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        feat_full = F.interpolate(feat2, size=disp.shape[-2:],
                                  mode="bilinear", align_corners=False)
        left01 = _to_unit_image(left)
        # Keep the tail numerically gentle across resolutions.
        disp_norm = disp / 192.0
        x = torch.cat([left01, disp_norm, feat_full], dim=1)
        h = self.trunk(x)
        edge = torch.sigmoid(self.edge_head(h))
        residual = torch.tanh(self.res_head(h)) * self.max_residual
        refined = F.softplus(disp + edge * residual)
        return refined, edge


@dataclass
class StereoLiteYoloCtxSharpConfig:
    base_ch: int = 24
    tile_feat_ch: int = 32
    ctx_ch: int = 32
    iters_16: int = 2
    ...
    sharp_hidden: int = 32
    sharp_max_residual: float = 2.0
    backbone: str = "yolo26s"     # mid-tier default; can switch to yolo26n
    backbone_pretrained: bool = True


class StereoLiteYoloCtxSharp(nn.Module):
    def __init__(self, cfg: StereoLiteYoloCtxSharpConfig | None = None):
        super().__init__()
        self.cfg = cfg or StereoLiteYoloCtxSharpConfig()
        ...
        self.sharp_tail = SharpRefineTail(
            feat_ch=ch2, hidden=self.cfg.sharp_hidden,
            max_residual=self.cfg.sharp_max_residual)
    ...
    def forward(self, left: torch.Tensor, right: torch.Tensor,
        ...
        d_pre_sharp = d_full
        d_full, edge_full = self.sharp_tail(d_full, left_in, fL2)

        if aux:
            return {
                "d_final": d_full,
                "d_pre_sharp": d_pre_sharp,
                "edge_full": edge_full,
                "d_half": d_half,
                "d4": tile.d,
                "d8": t8_stages[-1].d,
