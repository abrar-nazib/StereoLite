"""StereoLite_tilegru — current chassis + ConvGRU on the tile feat slot.

Replaces the stateless TileRefine trunk with a ConvGRU. The tile's feat
slot becomes the GRU hidden state, propagated across iterations within
a scale and across scales via plane-equation upsample (slope-aware).
...
from .tile_propagate import (TileState, TileInit, TileRefineGRU,
                              TileUpsample)
...
class StereoLite(nn.Module):
    def __init__(self, cfg: StereoLiteConfig | None = None):
        ...
        def make_refine(feat_ch):
            return TileRefineGRU(feat_ch=feat_ch,
