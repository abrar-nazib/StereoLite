"""StereoLite_yolo_ctx_init4 — YOLO26 context model with 1/4 cost warm start.
...
Pipeline:
  ...
  Plane upsample 1/8  → 1/4, blend with a fresh 1/4 cost-volume init,
  then iterate with TileRefineCtx(..., ctx=ctx4).

  Final: ConvexUpsample 1/4 → 1/2 → full, exactly as `StereoLite_yolo`.

The point is to keep the successful `yolo_ctx_gate` recurrent operator but
avoid asking a 1/16 warm start to preserve thin/far structure all the way to
full resolution.
"""
...
@dataclass
class StereoLiteYoloCtxInit4Config:
    base_ch: int = 24
    tile_feat_ch: int = 32
    ctx_ch: int = 32
    iters_16: int = 2
...
class StereoLiteYoloCtxInit4(nn.Module):
    def __init__(self, cfg: StereoLiteYoloCtxInit4Config | None = None):
        super().__init__()
        self.cfg = cfg or StereoLiteYoloCtxInit4Config()
        ...
        g4 = self.cfg.init_groups
        while ch4 % g4 != 0 and g4 > 1:
            g4 -= 1
        self.init_tile_4 = TileInit(feat_ch=ch4,
                                     max_disp=self.cfg.init4_max_disp,
                                     groups=g4,
                                     feat_out=self.cfg.tile_feat_ch)
        self.init4_blend = nn.Sequential(
            nn.Conv2d(self.cfg.ctx_ch + 4, self.cfg.refine_hidden, 3,
    ...
    def forward(self, left: torch.Tensor, right: torch.Tensor,
        ...
        init4 = self.init_tile_4(fL4, fR4)
        init4_delta = (init4.d - tile.d).abs()
        blend_in = torch.cat([ctx4, tile.conf, init4.conf, init4_delta,
                              tile.d], dim=1)
        blend = torch.sigmoid(self.init4_blend(blend_in))
        tile = TileState(
            d=F.softplus((1.0 - blend) * tile.d + blend * init4.d),
            sx=(1.0 - blend) * tile.sx,
            sy=(1.0 - blend) * tile.sy,
            feat=ctx4,
            conf=torch.maximum(tile.conf, init4.conf),
        )
        t4_stages = [tile]
