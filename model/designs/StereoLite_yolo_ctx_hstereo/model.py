"""StereoLite_yolo_ctx_hstereo — GEV4 plus true 1/2 stereo refinement.
...
Pipeline:
  ...
  Final: plane upsample 1/4 → 1/2, run TileRefineCtx on fL2/fR2, then
...
model from step 1. The extra 1/2 stereo stage is the important change here:
it lets the network re-check correspondence where thin structures still exist.
"""
...
@dataclass
class StereoLiteYoloCtxHStereoConfig:
    base_ch: int = 24
    tile_feat_ch: int = 32
    ctx_ch: int = 32
    iters_16: int = 2
...
class StereoLiteYoloCtxHStereo(nn.Module):
    def __init__(self, cfg: StereoLiteYoloCtxHStereoConfig | None = None):
        super().__init__()
        self.cfg = cfg or StereoLiteYoloCtxHStereoConfig()
        ...
        self.refine_4 = TileRefineCtx(
            ...
            half_range=self.cfg.cost_half_range, groups=g,
            ctx_ch=self.cfg.ctx_ch)
        self.refine_2 = TileRefineCtx(
            feat_ch=ch2, tile_feat_ch=self.cfg.tile_feat_ch,
            hidden=self.cfg.refine_hidden,
            half_range=self.cfg.cost_half_range, groups=g,
            ctx_ch=self.cfg.ctx_ch)
        ...
        # Final learned 2x upsample 1/2 → full after stereo refinement.
    ...
    def forward(self, left: torch.Tensor, right: torch.Tensor,
        ...
        # --- 1/4 → 1/2, then real high-resolution stereo refinement ---
        tile = self.up_4_to_2(tile, target_hw=fL2.shape[-2:])
        t2_stages = [tile]
        for _ in range(self.cfg.iters_2):
            tile = self.refine_2(tile, fL2, fR2, ctx2)
            t2_stages.append(tile)

        # --- Final convex upsample 1/2 → full ---
        d_half = tile.d
