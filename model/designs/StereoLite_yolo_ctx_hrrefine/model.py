"""StereoLite_yolo_ctx_hrrefine — YOLO26 context model + half-res refinement.
...
Pipeline:
  ...
  Final: ConvexUpsample 1/4 → 1/2, run a small edge-aware residual
  refinement loop at 1/2, then ConvexUpsample 1/2 → full.
...
from .tile_propagate import (
    TileState, TileInit, TileRefineCtx, TileUpsample,
    _correlation_lookup, _horizontal_warp,
)
...
class HalfResRefine(nn.Module):
    """Small residual GRU at 1/2 resolution.

    The 1/4 tile field is stable but can blur thin structures and depth
    discontinuities. This module only predicts a bounded residual around the
    convex-upsampled 1/2 disparity, using local correlation plus left-image
    context before the final 2x upsample.
    """

    def __init__(self, feat_ch: int, ctx_ch: int, hidden_ch: int = 32,
                 iters: int = 2, half_range: int = 2,
                 groups: int = 8, max_residual: float = 1.25):
        super().__init__()
        g = groups
        ...
        self.init_h = nn.Sequential(
            nn.Conv2d(feat_ch + ctx_ch + 1, hidden_ch, 3, padding=1,
                      bias=False),
            _gn(hidden_ch), nn.SiLU(inplace=True),
            nn.Conv2d(hidden_ch, hidden_ch, 3, padding=1, bias=False),
            _gn(hidden_ch), nn.SiLU(inplace=True),
        )
        inp_ch = 2 * feat_ch + ctx_ch + cost_ch + 3
        gru_ch = hidden_ch + inp_ch
        self.conv_z = nn.Conv2d(gru_ch, hidden_ch, 3, padding=1)
        self.conv_r = nn.Conv2d(gru_ch, hidden_ch, 3, padding=1)
        self.conv_q = nn.Conv2d(gru_ch, hidden_ch, 3, padding=1)
    ...
    def forward(self, d_half: torch.Tensor, fL2: torch.Tensor,
                fR2: torch.Tensor, ctx2: torch.Tensor) -> tuple[torch.Tensor,
                                                                 torch.Tensor]:
        h = self.init_h(torch.cat([fL2, ctx2, d_half], dim=1))
        d = d_half
        edge_acc = d.new_zeros(d.shape)
        for _ in range(self.iters):
            fR_w = _horizontal_warp(fR2, d)
            cost = _correlation_lookup(fL2, fR2, d, self.half_range,
                                       self.groups_eff)
            score = cost.view(
                cost.shape[0], self.groups_eff, 2 * self.half_range + 1,
            ...
            cost_entropy = -(prob * prob.clamp_min(1e-6).log()).sum(
                dim=1, keepdim=True)
            x = torch.cat([fL2, fR_w, ctx2, cost, d, cost_peak,
                           cost_entropy], dim=1)
            ...
            h = (1.0 - z) * h + z * q
            residual, edge_gate = self.delta_head(h).chunk(2, dim=1)
            edge_gate = torch.sigmoid(edge_gate)
            d = F.softplus(d + edge_gate * torch.tanh(residual)
                           * self.max_residual)
            edge_acc = edge_gate
        return d, edge_acc
...
@dataclass
class StereoLiteYoloCtxHRRefineConfig:
    base_ch: int = 24
    tile_feat_ch: int = 32
    ctx_ch: int = 32
    iters_16: int = 2
    ...
    hr_refine_hidden: int = 32
    hr_refine_iters: int = 2
    hr_refine_half_range: int = 2
    hr_refine_max_residual: float = 1.25
    backbone: str = "yolo26s"     # mid-tier default; can switch to yolo26n
    backbone_pretrained: bool = True


class StereoLiteYoloCtxHRRefine(nn.Module):
    def __init__(self, cfg: StereoLiteYoloCtxHRRefineConfig | None = None):
        super().__init__()
        self.cfg = cfg or StereoLiteYoloCtxHRRefineConfig()
        ...
        self.hr_refine = HalfResRefine(
            feat_ch=ch2, ctx_ch=self.cfg.ctx_ch,
            hidden_ch=self.cfg.hr_refine_hidden,
            iters=self.cfg.hr_refine_iters,
            half_range=self.cfg.hr_refine_half_range,
            groups=g,
            max_residual=self.cfg.hr_refine_max_residual,
        )
    ...
    def forward(self, left: torch.Tensor, right: torch.Tensor,
        ...
        d_half_pre_refine = d_half
        d_half, edge_half = self.hr_refine(d_half, fL2, fR2, ctx2)
        ...
        if aux:
            return {
                "d_final": d_full,
                "d_half": d_half,
                "d_half_pre_refine": d_half_pre_refine,
                "edge_half": edge_half,
                "d4": tile.d,
                "d8": t8_stages[-1].d,
