"""StereoLite_yolo_ctx_sru — YOLO26 context model with selective recurrence.
...
Pipeline:
  ...
  TileRefineCtx uses a selective recurrent unit: 1x1 updates preserve
  high-frequency boundaries while 3x3 updates propagate smooth context.
...
@dataclass
class StereoLiteYoloCtxSRUConfig:
    base_ch: int = 24
    tile_feat_ch: int = 32
    ctx_ch: int = 32
    iters_16: int = 2
...
class StereoLiteYoloCtxSRU(nn.Module):
    def __init__(self, cfg: StereoLiteYoloCtxSRUConfig | None = None):
        super().__init__()
        self.cfg = cfg or StereoLiteYoloCtxSRUConfig()
    ...
    def forward(self, left: torch.Tensor, right: torch.Tensor,
        ...
        if aux:
            return {
                ...
                "d16": t16_stages[-1].d,
                "d32": t16_stages[0].d,
                "iter_stages_16": [t.d for t in t16_stages],
                "iter_stages_8": [t.d for t in t8_stages],
                "iter_stages_4": [t.d for t in t4_stages],
            }
        return d_full
