"""Tile propagation for StereoLite_yolo_ctx_sru.

This builds on `StereoLite_yolo_ctx_gate`: keep the same cost lookup and
residual update gate, but replace the single fixed-kernel ConvGRU with a
Selective Recurrent Unit inspired by Selective-Stereo.

The update runs two GRU branches:
  - 1x1 branch: preserves high-frequency edges and thin structures.
  - 3x3 branch: propagates context across smooth/textureless regions.

A context/image attention map blends the branches per pixel before the
residual heads. This attacks over-smoothing at the iterative-update level
instead of trying to sharpen after the field is already blurred.
...
class TileRefineCtx(nn.Module):
    ...
    The GRU update is a two-branch selective unit. A small gate then decides
    how strongly to apply the predicted geometry update.
    """
    ...
    def __init__(self, feat_ch: int, tile_feat_ch: int, hidden: int = 48,
        ...
        self.conv_z_small = nn.Conv2d(gru_in, tile_feat_ch, 1)
        self.conv_r_small = nn.Conv2d(gru_in, tile_feat_ch, 1)
        self.conv_q_small = nn.Conv2d(gru_in, tile_feat_ch, 1)
        self.conv_z_large = nn.Conv2d(gru_in, tile_feat_ch, 3, padding=1)
        self.conv_r_large = nn.Conv2d(gru_in, tile_feat_ch, 3, padding=1)
        self.conv_q_large = nn.Conv2d(gru_in, tile_feat_ch, 3, padding=1)
        self.select_attn = nn.Sequential(
            nn.Conv2d(feat_ch + self.ctx_ch + 3, hidden, 3, padding=1,
                      bias=False),
    ...
    def forward(self, tile: TileState, fL: torch.Tensor,
        ...
        z_s = torch.sigmoid(self.conv_z_small(hctx))
        r_s = torch.sigmoid(self.conv_r_small(hctx))
        q_s = torch.tanh(self.conv_q_small(torch.cat([r_s * h, ctx_vec],
                                                     dim=1)))
        h_small = (1.0 - z_s) * h + z_s * q_s

        z_l = torch.sigmoid(self.conv_z_large(hctx))
        r_l = torch.sigmoid(self.conv_r_large(hctx))
        q_l = torch.tanh(self.conv_q_large(torch.cat([r_l * h, ctx_vec],
                                                     dim=1)))
        h_large = (1.0 - z_l) * h + z_l * q_l

        # Large image/context gradients favour the 1x1 branch; flat regions
        # favour the 3x3 branch. The learned attention can override this
        # heuristic, but the gradient prior gives it a useful starting bias.
        ctx_energy = ctx.abs().mean(dim=1, keepdim=True)
        img_energy = fL.abs().mean(dim=1, keepdim=True)
        gx = F.pad((img_energy[..., :, 1:] - img_energy[..., :, :-1]).abs(),
                   (0, 1, 0, 0))
        gy = F.pad((img_energy[..., 1:, :] - img_energy[..., :-1, :]).abs(),
                   (0, 0, 0, 1))
        attn_in = torch.cat([fL, ctx, tile.conf, ctx_energy,
                             (gx + gy)], dim=1)
        edge_attn = torch.sigmoid(self.select_attn(attn_in))
        h_new = edge_attn * h_small + (1.0 - edge_attn) * h_large
