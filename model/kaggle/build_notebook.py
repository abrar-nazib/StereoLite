...
cells.append(md(
    ...
    "   ```\n"
    "   /kaggle/input/<your-dataset-slug>/frames_finalpass/15mm_focallength/scene_*/fast|slow/left|right/*.png\n"
    "   /kaggle/input/<your-dataset-slug>/disparity/15mm_focallength/scene_*/fast|slow/left/*.pfm\n"
    "   ```\n"
    "   Set `DATA_ROOT` in the **Config** cell below to your dataset's path "
    "(e.g. `/kaggle/input/sceneflow-driving`).\n"
...
cells.append(md(
    "## 2b. Download Scene Flow Driving to `/kaggle/working/`\n"
    "\n"
    "Scene Flow Driving is **not** a default Kaggle asset. Two options:\n"
    "\n"
    "**Option A (recommended): attach a Kaggle Dataset.** If you already "
    "uploaded Scene Flow Driving as a Kaggle Dataset (Settings > Input > "
    "Add Input), skip this cell — the Config cell below will auto-detect it.\n"
    "\n"
    "**Option B: download from Freiburg** (the official source). This cell "
    "fetches the two tarballs (~15 GB total: ~14 GB finalpass RGB + "
    "~0.5 GB disparity) and extracts them into `/kaggle/working/"
    "sceneflow_driving/`. Takes 10-30 min depending on Kaggle's outbound "
    "bandwidth. Only the `frames_finalpass` + `disparity` subsets are "
    "needed — we skip `cleanpass`, `optical_flow`, `material`, etc.\n"
    "\n"
    "Skip this cell if you chose Option A.\n"
))
cells.append(code(
    "import os, subprocess\n"
    "\n"
    "TARGET = '/kaggle/working/sceneflow_driving'\n"
    "BASE = 'https://lmb.informatik.uni-freiburg.de/data/SceneFlowDatasets_CVPR16/Release_april16/data/Driving'\n"
    "FRAMES_URL = f'{BASE}/raw_data/driving__frames_finalpass.tar'\n"
    "DISP_URL   = f'{BASE}/derived_data/driving__disparity.tar.bz2'\n"
    "\n"
    "os.makedirs(TARGET, exist_ok=True)\n"
    "\n"
    "def dl(url, to):\n"
    "    if os.path.exists(to) and os.path.getsize(to) > 1_000_000:\n"
    "        print(f'  already have {os.path.basename(to)}  '\n"
    "              f'({os.path.getsize(to)/1e9:.2f} GB)')\n"
    "        return\n"
    "    print(f'  downloading {os.path.basename(to)} ...')\n"
    "    rc = subprocess.call(['wget', '-q', '--show-progress', '-O', to, url])\n"
    "    if rc != 0 or os.path.getsize(to) < 1_000_000:\n"
    "        raise RuntimeError(f'download failed ({url}) — rc={rc}')\n"
    "    print(f'  got {os.path.getsize(to)/1e9:.2f} GB')\n"
    "\n"
    "def extract(tar, to):\n"
    "    flag = 'xjf' if tar.endswith('.bz2') else 'xf'\n"
    "    print(f'  extracting {os.path.basename(tar)} ...')\n"
    "    # No strip: let auto-detect find frames_finalpass wherever it lands.\n"
    "    subprocess.check_call(['tar', flag, tar, '-C', to])\n"
    "\n"
    "print('== Scene Flow Driving download ==')\n"
    "if not os.path.isdir(os.path.join(TARGET, 'frames_finalpass')):\n"
    "    # Check deeper in case already extracted\n"
    "    import glob\n"
    "    hit = glob.glob(os.path.join(TARGET, '**', 'frames_finalpass'), recursive=True)\n"
    "    if not hit:\n"
    "        tar_frames = os.path.join(TARGET, 'driving__frames_finalpass.tar')\n"
    "        dl(FRAMES_URL, tar_frames)\n"
    "        extract(tar_frames, TARGET)\n"
    "        os.remove(tar_frames)\n"
    "\n"
    "if not os.path.isdir(os.path.join(TARGET, 'disparity')):\n"
    "    import glob\n"
    "    hit = glob.glob(os.path.join(TARGET, '**', 'disparity'), recursive=True)\n"
    "    if not hit:\n"
    "        tar_disp = os.path.join(TARGET, 'driving__disparity.tar.bz2')\n"
    "        dl(DISP_URL, tar_disp)\n"
    "        extract(tar_disp, TARGET)\n"
    "        os.remove(tar_disp)\n"
    "\n"
    "print('\\n-- resulting layout --')\n"
    "for root, dirs, files in os.walk(TARGET, followlinks=True):\n"
    "    depth = root.replace(TARGET, '').count('/')\n"
    "    if depth <= 2:\n"
    "        print(' ', root)\n"
    "    if depth >= 2:\n"
    "        dirs[:] = []\n"
...
cells.append(code(
    "import os, glob\n"
    "\n"
    "# ---- EDIT THIS if auto-detect misses it ---------------------------\n"
    "DATA_ROOT = '/kaggle/input/sceneflow-driving'\n"
    "# -------------------------------------------------------------------\n"
    "\n"
    "EPOCHS        = 30\n"
    "BATCH_PER_GPU = 4       # 4 fits on a T4 at 384x768; drop to 2 if OOM\n"
    "INF_H         = 384\n"
    "INF_W         = 768\n"
    "LR_PEAK       = 8e-4\n"
    "RANDOM_ERASE  = 0.3\n"
    "\n"
    "# Auto-detect: walk /kaggle/input/ up to 4 levels deep looking for\n"
    "# a directory called frames_finalpass. DATA_ROOT is its parent.\n"
    "if not os.path.isdir(os.path.join(DATA_ROOT, 'frames_finalpass')):\n"
    "    hit = None\n"
    "    for pat in (\n"
    "        '/kaggle/input/*/frames_finalpass',\n"
    "        '/kaggle/input/*/*/frames_finalpass',\n"
    "        '/kaggle/input/*/*/*/frames_finalpass',\n"
    "        '/kaggle/input/*/*/*/*/frames_finalpass',\n"
    "    ):\n"
    ...
    "            break\n"
    "    if hit:\n"
    "        DATA_ROOT = os.path.dirname(hit)\n"
    "        print(f'Auto-detected DATA_ROOT = {DATA_ROOT}')\n"
    "    else:\n"
    "        print('*** Could not locate frames_finalpass/ anywhere under '\n"
    "              '/kaggle/input/. Contents follow — set DATA_ROOT manually:')\n"
    ...
    "            if depth >= 3:\n"
    "                dirs[:] = []\n"
    "        raise FileNotFoundError('frames_finalpass/ not found — see tree above')\n"
    "\n"
    "# Quick reachability check\n"
    "disp_root = os.path.join(DATA_ROOT, 'disparity')\n"
    "if not os.path.isdir(disp_root):\n"
    "    print(f'warning: expected disparity/ next to frames_finalpass/ under\\n'\n"
    "          f'         {DATA_ROOT}\\n'\n"
    "          f'but not found. Scene Flow Driving needs both frames_finalpass/ '\n"
    "          f'and disparity/ in the same parent.')\n"
    "\n"
